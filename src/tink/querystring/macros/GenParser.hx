...
class GenParser {
	...
	function new(name, rawType:Type, pos) {
		this.pos = pos;
		this.name = name;
		this.resultType = switch rawType.reduce() {
		...
		if (this.value == null) {
			if (this.valueType == null) {
				...
				this.value = this.valueType.toComplex();
		}
		...
		if (this.input == null) {
			if (this.inputType == null) {
				...
				this.input = this.inputType.toComplex();
		}

		// Now comes the sad part - see tink.Stringly for further rants ...
		this._string = if ((macro((null : $value) : String)).typeof()
			.isSuccess()) prim(macro:String); else pos.error('${value.toString()} should be compatible with String');
		...
		function parsePrimitive(expected:ComplexType)
			return if ((macro((null : $value) : $expected)).typeof()
				.isSuccess()) prim(macro:Int); else if ((macro((null : $value) : tink.Stringly)).typeof()
				.isSuccess()) coerce(macro ${prim(macro:tink.Stringly)}, expected); else coerce(macro(${prim(macro:String)} : tink.Stringly), expected);
  ...
  public function dyn(e, ct) 
	...
	public function enumAbstract(names:Array<Expr>, e:Expr, ct:ComplexType, pos:Position):Expr {
		return macro @:pos(pos) {
			var v:$ct = cast $e;
			${
				ESwitch(macro v, [{expr: macro v, values: names}], macro {
					var list = $a{names};
	...
	public function rescue(t:Type, pos:Position, gen:GenType):Option<Expr> {
		return Some(macro try
			${prim(t.toComplex())} catch (e:tink.core.Error)
			this.fail(prefix, e.message)
		catch (e:Dynamic)
			this.fail(prefix, Std.string(e)));
	}
